...
export default function PresentationDashboard() {
  ...
  const extractTitle = (presentation: any) => {
  ...
  const extractSlideCount = (presentation: any) => {
    if (presentation.generated_content) {
      try {
        const content = JSON.parse(presentation.generated_content)
        if (content.slides) {
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {presentations.map((presentation) => (
                <Link key={presentation.id} href={`/presentation/${presentation.id}/preview`} className="block group">
                  <Card className="h-full overflow-hidden border border-gray-200 hover:border-blue-300 hover:shadow-lg transition-all duration-200 group-hover:transform group-hover:-translate-y-1">
                    <div className="h-40 bg-gradient-to-br from-blue-50 to-cyan-50 border-b border-gray-100 flex items-center justify-center">
                      {presentation.generated_content ? (
                        <PresentationIcon className="w-16 h-16 text-blue-400 group-hover:text-blue-500 transition-colors" />
                      ) : (
                        <FileIcon className="w-16 h-16 text-gray-300 group-hover:text-gray-400 transition-colors" />
                      )}
                    </div>
                    <CardContent className="p-5">
                      ...
                      <p className="text-gray-500 text-sm line-clamp-2 mb-4">
                        {presentation.content_input || "No description available"}
                      </p>
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        ...
                        {extractSlideCount(presentation) > 0 && (
                          <div className="flex items-center gap-1">
                    ...
                    <CardFooter className="px-5 py-3 bg-gray-50 border-t border-gray-100">
                      <div className="w-full flex justify-between items-center">
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          className="text-blue-600 hover:text-blue-700 hover:bg-blue-50 p-0 h-8 px-2"
                        >
